...
It hosts the services my family uses daily: a [Nextcloud](https://nextcloud.com/) instance, a [Vaultwarden](https://github.com/dani-garcia/vaultwarden) password manager, a [Gitea](https://gitea.io/) git server, and a [Calibre-Web](https://github.com/janeczku/calibre-web) library.
For a long time I had no proper backup for any of this.
That was fine when it was just me playing around, but once my family started relying on these services, I knew I had to do something.
...
- **Tightly coupled.** The restore process depended on the production server being reachable over SSH. If production was down, restoring was painful.
...
- **Bad for security.** Because `srv-test-2` SSHed directly into production as the service users (`nextcloud`, `git`, `vaultwarden`, `calibre-web`), I had to give each of those accounts a real login shell and authorize the test server's SSH key for all of them. Service accounts should not have shell access. It works, but it expands the attack surface in a way that feels wrong.

I knew I had to level up.
...
**srv-prod-2** runs the services and pushes restic backups to a [MinIO](https://min.io/) instance on **srv-prod-3**.
...
- **3** copies of the data (production, srv-prod-3, srv-offsite-1)
...
The NixOS `services.restic.backups` module handles the production side nicely.
Here is a simplified version of the backup configuration from [`srv-prod-2`](https://github.com/britter/nix-configuration/blob/main/systems/x86_64-linux/srv-prod-2/configuration.nix):
...
One extra thing the module does is generate a wrapper script for each restore job.
...
With the module in place, the restore configuration on [`srv-test-2`](https://github.com/britter/nix-configuration/blob/main/systems/x86_64-linux/srv-test-2/configuration.nix) mirrors the backup configuration closely:
...
- [`modules/restic-restore/default.nix`](https://github.com/britter/nix-configuration/blob/main/modules/restic-restore/default.nix) - the custom restore module
- [`systems/x86_64-linux/srv-prod-2/configuration.nix`](https://github.com/britter/nix-configuration/blob/main/systems/x86_64-linux/srv-prod-2/configuration.nix) - the backup configuration
- [`systems/x86_64-linux/srv-test-2/configuration.nix`](https://github.com/britter/nix-configuration/blob/main/systems/x86_64-linux/srv-test-2/configuration.nix) - the restore configuration
- [`systems/x86_64-linux/srv-offsite-1/configuration.nix`](https://github.com/britter/nix-configuration/blob/main/systems/x86_64-linux/srv-offsite-1/configuration.nix) - the off-site sync configuration
